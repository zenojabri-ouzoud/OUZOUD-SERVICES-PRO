...
def mobile_barcode_scanner(session_key): 
    """Enhanced barcode scanner that ensures value is written to st.text_input"""
    scanner_html = f""" 
    ...
    (function() {{ 
        ...
        function setInputValue(value) {{ 
            ...
            // محاولة 4: البحث عن أي input يحتوي placeholder يشبه الباركود
            if (!input) {{ 
        ...
        function sendToStreamlit(value) {{ 
            ...
            // إرسال مرة ثانية بعد ms 50 للتأكد
            setTimeout(() => {{ 
                window.parent.postMessage({{ 
                    type: 'streamlit:setComponentValue', 
                    key: '{session_key}', 
            ...
            // إرسال مرة ثالثة بعد ms 200 (ضمان إضافي)
            setTimeout(() => {{ 
                window.parent.postMessage({{ 
                    type: 'streamlit:setComponentValue', 
                    key: '{session_key}', 
...
def generate_facture_80mm(cart_data, titre="FACTURE"): 
    """
    Unified 80mm invoice for all operations (sale, printing, services, orders)
    with automatic sequential invoice number
    """
...
def generate_commande_pdf(commandes_data): 
    """Order invoice using unified design"""
    cart_data = [] 
...
translations = { 
    ...
    "reset_warning": { 
        ...
        "fr": "Ce bouton enregistrera le résumé du jour et remettra le compteur à zéro. À utiliser uniquement en fin de journée!", 
...
def export_import_buttons(table_name, data_df): 
    """Export and Import Excel buttons"""
...
def reset_caisse(): 
    """Reset cash register and save today's summary"""
...
def add_to_credit(credit_id, montant_addition): 
    """Add amount to credit (increase debt)"""
    credit_actuel = supabase.table("credits").select("*").eq("id", int(credit_id)).execute().data[0] 
...
def auto_cart_scanner(): 
    """Auto cart scanner that adds product to cart automatically with product recognition"""
    scanner_html = """ 
...
def stock_barcode_scanner(target_input_label): 
    """Barcode scanner that fills only a specific field"""
    scanner_html = f""" 
...
def voice_command_component(): 
    ...
    <div id="voice-control"> 
        <button id="start-voice" style="padding:10px 20px; background:#4CAF50; color:white; border:none; border-radius:5px; cursor:pointer; font-size:16px;">ابدأ الاستماع</button> 
        <button id="stop-voice" style="padding:10px 20px; background:#f44336; color:white; border:none; border-radius:5px; cursor:pointer; font-size:16px; display:none;">إيقاف</button> 
...
if menu == t("pos"): 
    ...
    if st.session_state.auto_sale_mode: 
        ...
        # ====== Cart ====== 
            ...
            # Choose cart type: manual or auto
            cart_mode = st.radio( 
                t("cart_mode_label"), 
...
# ==================== STOCK ====================
    ...
    # Add product with barcode scanner
...
# ==================== CREDITS ====================
    ...
    if not df_credits.empty: 
        ...
        # ========== 3 Buttons: Add / Pay / Delete ========== 
        col_add, col_pay, col_delete = st.columns(3) 
         
        # Add to credit button (increase amount)
        with col_add: 
        ...
        # Delete credit button (permanently)
        with col_delete: 
            if st.button(t("delete_credit"), use_container_width=True, key="delete_existing_credit_btn"): 
                if credit_a_reduire: 
                    try: 
                        ...
                        st.warning(f"هل أنت متأكد من حذف دين **{credit_data['Client']}** بمبلغ **{credit_data['Montant']:.2f} DH** ؟") 
...
# ==================== SERVICES ====================
    ...
    if "selected_service" in st.session_state and st.session_state.selected_service: 
        ...
        # Complete service and print invoice button
