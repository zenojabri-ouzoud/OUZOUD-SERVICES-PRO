...
#4CAF50; border-radius:10px; padding:15px; background:#f9f9f9;"> 
        ...
        <p style="text-align:center; color:#666; font-size:14px; margin-top:10px;">         قرب الباركود من >p/<الكاميرا

        <div id="scan-status" style="text-align:center; font-size:14px; color:#999; margin-top:5px;">        >div/<...جاري تهيئة الكامير
ا
    </div> 
...
|| el.placeholder.includes('Code'))) {{ 
        ...
        function sendToStreamlit(value) {{  Streamlit إرسال القيمة لـ //           
            ...
            window.parent.postMessage({{ 
              ...
              للتأكد ms 50 إرسال مرة ثانية بعد   //           
            ...
            setTimeout(() => {{ 
              ...
              )ضمان إضافي( ms 200 إرسال مرة ثالثة بعد  //           
 ...
 ثانية 2 إعادة تشغيل الماسح بعد  .4 //                    
                    setTimeout(() => {{ 
                        lastScanned = ''; 
                        startScanner(); 
                    }}, 2000); 
 ...
 إعادة تعيين المتغيرات  //                
                ...
                html5Qrcode.start( 
                    ...
                    function(errorMessage) {{  األخطاء العادية  تجاهل  //                       
 
                    }} 
                ).then(function() {{ 
                    isScanning = true; 
                    updateStatus('          امسح الباركود - الكاميرا شغالة ', false); 
                }}).catch(function(err) {{ 
                    ...
                    if (retryCount < maxRetries) {{ 
                        ...
                        updateStatus('❌ تعذر تشغيل الكاميرا بعد ' + maxRetries + ' محاوالت', false, true); 
                    }} 
                }}); 
            }} catch(e) {{ 
  ...
  ==================== مع رقم تسلسلي  mm 80 دالة الفاتورة الموحدة   ==================== 
#
def get_next_invoice_number():  """الحصول على رقم الفاتورة التالي"""   
    ...
    if 'Facture' in df_ventes.columns: 
      ...
      1 يكن هناك أرقام فواتير، ابدأ من إذا لم  #   
...
mm  )لجميع العمليات (بيع، طباعة، خدمات، طلبيات  مع رقم فاتورة تسلسلي تلقائ
ي
""" 
invoice_number = get_next_invoice_number() 
pdf = FPDF('P', 'mm', (80, 297)) 
pdf.add_page() 
pdf.set_auto_page_break(auto=True, margin=5)  ========== الرأس  ========== 
#
...
pdf.cell(70, 4, "-" * 40, ln=True, align='C')  ========== التاريخ والوقت ========== 
#
...
pdf.cell(70, 4, "-" * 40, ln=True, align='C')  ========== جدول المنتجات ========== 
#
...
def generate_pdf_80mm(cart_data):  """فاتورة البيع والخدمات"""   
 
    return generate_facture_80mm(cart_data, "FACTURE DE VENTE") 
 
def generate_impression_pdf(prix_page, nombre):  """ تستخدم نفس التصميم الموحد -فاتورة الطباعة  """   
...
def generate_commande_pdf(commandes_data):  """ تستخدم نفس التصميم الموحد  -فاتورة الطلبية  """   
...
def generate_pdf(cart_data):  """دالة عامة للفاتورة"""   
...
AAAAA="> 
  ...
  --- (العربية، الفرنسية، اإلنجليزية) نظام الترجمة  --- 
#
...
translations = { 
    ...
    "auto_sale_mode": { 
        "ar": "  البيع التلقائي (  80 سكانير = بيع مباشر + فاتورة mm)", 
    ...
    "cart_auto_info": { 
        "ar": "    = الماسح التلقائي: امسح الباركود = يضاف للسلة تلقائيا ً | امسح باركود آخر = يضاف أيضا ً | اضغط إنهاء ,"فاتورة
...
Facture", 
    ...
    "cart_empty": { 
        "ar": " امسح الباركود لإلضافة - لة فارغة الس ", 
    ...
    "search_stock": { 
        "ar": "    المخزون (باالسم أو الباركود) بحث في ", 
    ...
    "reset_warning": { 
        "ar": "    هذا الزر سيحفظ ملخص اليوم ويصفر العداد. استخدمه فقط في نهاية اليوم!", 
...
uniquement en fin de journée!", 
    ...
    "reduce_credit_info": { 
        "ar": " ًهذا الزر يقلل من مبلغ الدين بدون حذفه كامل", 
    ...
    "no_pending_orders": { 
        "ar": "ال توجد طلبيات قيد االنتظار", 
    ...
    "order_received": { 
        "ar": "✅ كيد االستلم وتحديث المخزون تم تأ !", 
    ...
    "invoice_printed": { 
        "ar": "    ً تمت طباعة الفاتورة تلقائيا", 
...
def t(key):  """ مفتاح إلى اللغة المختارة ترجمة ال """   
...
def export_import_buttons(table_name, data_df):  """Excel أزرار تصدير واستيراد"""   
...
def get_product_info(code_or_name):  """ تج بالباركود أو االسم البحث عن من """   
...
def reset_caisse():  """ ثم مسح جميع بيانات اليوم -تصفير الخزينة وحفظ ملخص اليوم  """   
...
df_impressions.columns else 0 
    total_jour = total_ventes + total_impressions 
      باش تجنب الخطأ  try/except استخدم -وم في التاريخ  حفظ ملخص الي  #   
    ...
    except Exception as e:  إذا كانت الجدول ماكاينش، تجاهل الخطأ  #       
...
def add_to_credit(credit_id, montant_addition):  """ للدين (زيادة الدين) إضافة مبلغ """   
...
def auto_cart_scanner():  """ يضيف المنتج للسلة تلقائيا ً مع التعرف على المنتج الجديد -سكانير السلة التلقائية """   
...
def stock_barcode_scanner(target_input_label): 
 """ يعمل بشكل صحيح -ماسح باركود يمأل خانة محددة فقط  """    
    scanner_html = f""" 
  ...
  ==================== نظام التحكم الصوتي ==================== 
#
def voice_command_component():  """مكون التحكم الصوتي"""   
 
    voice_html = """ 
    <div id="voice-control"> 
        <button id="start-voice" style="padding:10px 20px; background:#4CAF50; color:white; 
border:none; border-radius:5px; cursor:pointer; font-size:16px;">  ابدأ االستماع                
 
        </button> 
        <button id="stop-voice" style="padding:10px 20px; background:#f44336; color:white; 
border:none; border-radius:5px; cursor:pointer; font-size:16px; display:none;">  إيقاف               
    ...
    document.getElementById('start-voice').addEventListener('click', function() { 
        ...
        document.getElementById('voice-status').innerText = '    جاري االستماع...'; 
    }); 
...
btn.innerText.includes('طباعة')) { 
            ...
            const pageMap = {  ,' التحكم لوحة       لوحة التحكم': ' '               
  ,' البيع نقطة       نقطة البيع': ' '               
 ...
 ,' الموردين طلبيات          الطلبيات': ' '                 ,' اإللكترونية الخدمات  الخدمات': ' '               
  ' سريعة  أدوات    األدوات': ' '               
...
df_v.columns else pd.DataFrame() 
    ...
    with col4: 
        st.metric("    مخزون منخفض", low_stock) 
    ...
    if not df_v.empty: 
        ...
        if not top_products.empty: 
            fig = px.bar(x=top_products.index, y=top_products.values, title="  منتجات  10 أفضل ", labels={'x':  )}')DH( المبيعات' :'y' ,'المنتج 
'
...
date2_end)]['Total'].sum() 
    ...
    if not df_v.empty: 
        ...
        fig = px.line(daily_sales.tail(30), x='التاريخ', y='المبيعات', title="  يوم) 30 تطور المبيعات (آخر ") 
        st.plotly_chart(fig, use_container_width=True) 
    ...
    if not df_v.empty: 
        df_v['Date_dt'] = pd.to_datetime(df_v['Date'], format='%d/%m/%Y %H:%M', errors='coerce') 
        daily_sales = df_v.groupby(df_v['Date_dt'].dt.date)['Total'].sum().reset_index() 
        daily_sales.columns = [' يخ', 'المبيعات التار '] 
        ...
        if len(daily_sales) > 7: 
            ...
            with col_t1: 
                st.metric(" ( 7 متوسط المبيعات )أيام ", f"{avg_sales:.0f} DH") 
            with col_t2: 
...
{product['Quantité']}") 
        ...
        if mode == t("normal_sale"): 
            ...
            if use_normal_scanner: 
                st.info("           سيتم كتابته تلقائيا ً   -امسح الباركود اآلن ") 
...
Facture: {invoice_number}") 
        ...
        # ====== Cart ====== 
        elif mode == t("cart"):  اختيار نوع السلة: يدوي أو تلقائي  #           
            ...
            if cart_mode == t("cart_manual"): 
                ...
                with col1: 
                    ...
                    if st.button(t("add_to_cart")): 
                        if code and qty > 0 and prix_u > 0:  التحقق إذا كان المنتج موجودا ً مسبقا ً في السلة  #                           
...
VENTE") 
              ...
              ========== السلة التلقائية (سكانير متواصل)  ========== #           
                ...
                if code_auto_cart: 
                    ...
                    if product: 
                        if float(product['Quantité']) >= 1:  التعرف على المنتج الجديد تلقائيا ً  #                           
...
VENTE") 
    ...
    if search_term and not df_stock.empty: 
        ...
        mask_code = df_stock['Code-barres'].str.contains(search_term, case=False, na=False) if 'Code
barres' in df_stock.columns else pd.Series([False] * len(df_stock)) 
    ...
    with st.expander(t("add_product"), expanded=True): 
        ...
        if use_add_scanner: 
            st.info("           سيتم كتابته تلقائيا ً في خانة الباركود  -امسح الباركود اآلن ") 
...
key="update_scanner_checkbox") 
            if use_update_scanner: 
                st.info("         سيتم كتابته تلقائيا ً في خانة الباركود   -سح الباركود اآلن  ام ") 
...
df_credits_caisse.columns else 0 
    ...
    st.error("     تحذير: هذا الزر سيحفظ ملخص اليوم ))" ثم يمسح جميع بيانات اليوم (مبيعات، طباعة، ديون 
    ...
    if st.session_state.caisse_reset_confirmed: 
        st.error("❌ سح جميع بيانات اليوم نهائيا ً هل أنت متأكد؟ سيتم م !") 
...
0 
    ...
    search_credit = st.text_input(  ,":ابحث باسم العميل "       
    ...
    if search_credit and not df_credits.empty: 
        ...
        if df_credits.empty: 
            st.info(f"ال توجد نتائج لـ '{search_credit}'") 
        else: 
    ...
    if not df_credits.empty: 
        ...
        with col_credit2: 
            montant_operation = st.number_input(  ,"المبلغ"               
...
key="add_to_existing_credit_btn"): 
                if credit_a_reduire and montant_operation > 0: 
                    try: 
                        ...
                        st.success(f"✅ تمت إضافة {montant_operation:.2f} DH | حالي الدين ال : {nouveau:.2f} DH") 
                        play_success_sound() 
                        st.rerun() 
                    except Exception as e: 
                        st.error(f"{t('error_generic')}: {str(e)}") 
            ...
            if st.button(t("delete_credit"), use_container_width=True, key="delete_existing_credit_btn"): 
                if credit_a_reduire: 
                    try: 
                        ...
                        st.warning(f"    هل أنت متأكد من حذف دين **{credit_data['Client']}** بمبلغ 
**{credit_data['Montant']:.2f} DH** ؟") 
                        ...
                        with col_confirm_del: 
                            if st.button("✅ نعم، احذف", key="confirm_delete_credit_btn"): 
                                supabase.table("credits").delete().eq("id", credit_id).execute() 
                                st.success(f"✅  ً تم حذف الدين نهائيا") 
                                play_success_sound() 
                                time.sleep(0.5) 
                                st.rerun() 
                        with col_cancel_del: 
...
# ==================== SERVICES ==================== 
    ...
    if not df_services_db.empty: 
        ...
        st.info("ال توجد خدمات. أضف خدمات جديدة من األعلى.") 
...
key="service_confirm_btn"):  تحضير بيانات الفاتورة  #           
...
Facture: {invoice_number}") 
            ...
            if os.path.exists("facture_80mm.pdf"): 
                with open("facture_80mm.pdf", "rb") as f: 
                    st.download_button(  ,"تحميل الفاتورة     "                       
