...
def mobile_barcode_scanner(session_key):
 """
input_text.st ماسح باركود محسن - يضمن كتابة القيمة في
 """
 scanner_html = f"""
 <div id="barcode-scanner-container" style="width:100%; min-height:350px; border:2px dashed
#4CAF50; border-radius:10px; padding:15px; background:#f9f9f9;">
 <div id="reader" style="width:100%; min-height:300px;"></div>
 <p style="text-align:center; color:#666; font-size:14px; margin-top:10px;"> من الباركود قرب
<p/<الكاميرا
 <div id="scan-status" style="text-align:center; font-size:14px; color:#999; margin-top:5px;">
<div/<...جاري تهيئة الكاميرا
 </div>
...
id بالـ input محاولة :1 البحث عن //
 let input = document.getElementById('{session_key}');

name بالـ input محاولة :2 البحث عن //
 if (!input) {{
 const inputs = document.getElementsByName('{session_key}');
 if (inputs.length > 0) input = inputs[0];
 }}

label-aria بالـ input محاولة :3 البحث عن //
 if (!input) {{
 const inputs = document.querySelectorAll('input[aria-label="{session_key}"]');
 if (inputs.length > 0) input = inputs[0];
 }}

يشبه الباركود placeholder يحتوي على input محاولة :4 البحث عن أي //
 if (!input) {{
 const allInputs = document.querySelectorAll('input');
 for (let el of allInputs) {{
 if (el.placeholder && (el.placeholder.includes('باركود || ('el.placeholder.includes('barcode')
...
للتأكد msإرسال مرة ثانية بعد 50 //
 setTimeout(() => {{
 window.parent.postMessage({{
 type: 'streamlit:setComponentValue',
 key: '{session_key}',
...
(ضمان إضافي) msإرسال مرة ثالثة بعد 200 //
 setTimeout(() => {{
 window.parent.postMessage({{
 type: 'streamlit:setComponentValue',
 key: '{session_key}',
 ...
 updateStatus('✅المسح تم: ' + decodedText, true);

input كتابة القيمة في 1. //
 const inputUpdated = setInputValue(decodedText);

Streamlit إرسال القيمة لـ 2. //
...
;(false ,'جاهز للمسح مرة أخرى ')updateStatus

إعادة تشغيل الماسح بعد 2 ثانية 4. //
 setTimeout(() => {{
 lastScanned = '';
 startScanner();
 }}, 2000);
...
Stock مهم لـ - (rerun (إرسال طلب تحديث الصفحة 5. //
 window.parent.postMessage({{
 type: 'streamlit:setComponentValue',
 key: '{session_key}',
 value: decodedText
...
;(false ,'...جاري تشغيل الكاميرا ')updateStatus
...
تجاهل األخطاء العادية //
 }}
 ).then(function() {{
 isScanning = true;
;(false ,'الكاميرا شغالة - امسح الباركود ')updateStatus
 }}).catch(function(err) {{
...
;(true ,false ,'محاوالت ' + maxRetries + ' تعذر تشغيل الكاميرا بعد❌')updateStatus
 }}
 }});
 }} catch(e) {{
...
==================== مع رقم تسلسلي mmدالة الفاتورة الموحدة 80 ==================== #
...
def generate_facture_80mm(cart_data, titre="FACTURE"):
 """
لجميع العمليات )بيع، طباعة، خدمات، طلبيات( mmفاتورة موحدة 80
...
==================== الدوال المعدلة الستخدام الفاتورة الموحدة ==================== #
...
def play_success_sound():
 ...
 <audio autoplay>
 <source
src="data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQ
AAAAA=">
 </audio>
 """
 components.html(sound_html, height=0)
--- نظام الترجمة )العربية، الفرنسية، اإلنجليزية( --- #
...
,"أدوات سريعة " :"ar "
 ...
 "activate_scanner": {
,"تفعيل الماسح الضوئي السريع " :"ar "
 "fr": " Activer le scanner rapide",
 "en": " Activate Fast Scanner"
 },
 "auto_sale_mode": {
,"(mmسكانير = بيع مباشر + فاتورة 80) البيع التلقائي " :"ar "
...
,":نوع السلة " :"ar "
 ...
 "cart_manual": {
,"يدوي )إضافة منتج منتج( " :"ar "
 "fr": " Manuel (Ajout produit par produit)",
 "en": " Manual (Add one by one)"
 },
 "cart_auto": {
,"تلقائي )سكانير متواصل( " :"ar "
 "fr": " Auto (Scan continu)",
 "en": " Auto (Continuous scan)"
 },
 "cart_auto_info": {
| اضغط إنهاء = " :"ar "
| امسح باركود آخر = يضاف أيضاً
الماسح التلقائي: امسح الباركود = يضاف للسلة تلقائياً
,"فاتورة
 "fr": " Scanner auto: Scannez = Ajouté au panier | Scannez autre = Ajouté aussi | Terminer =
Facture",
 "en": " Auto scanner: Scan = Added to cart | Scan another = Added too | Finish = Invoice"
 },
 "cart_empty": {
,"السلة فارغة - امسح الباركود لإلضافة" :"ar "
...
,"أضف إلى السلة ✅" :"ar "
 ...
 "validate_cart": {
,"تأكيد وتسجيل الكل " :"ar "
 "fr": " Valider et Enregistrer Tout",
 "en": " Validate and Save All"
 },
 "clear_cart": {
,"تفري غ السلة " :"ar "
 "fr": " Vider le Panier",
 ...
 "finish_cart": {
,"إنهاء وإصدار الفاتورة " :"ar "
 "fr": " Terminer et Imprimer la Facture",
 "en": " Finish & Print Invoice"
 },
 "sale_success": {
,"!تم تسجيل البيع بنجاح✅" :"ar "
 "fr": "✅Vente enregistrée avec succès!",
 "en": "✅Sale recorded successfully!"
 },
 "low_stock_warning": {
,":المخزون غير كافي! المتوفر " :"ar "
 "fr": " Stock insuffisant! Disponible:",
 "en": " Insufficient stock! Available:"
 },
 "product_not_found": {
,"المنتج غير موجود في المخزون " :"ar "
 "fr": " Produit introuvable dans le stock",
 "en": " Product not found in stock"
 },
 "add_product": {
,"إضافة منتج➕" :"ar "
...
,"المجموع العام " :"ar "
 ...
 "reset_caisse": {
,"تصفير الخزينة )نهاية اليوم( " :"ar "
 "fr": " Réinitialiser la Caisse (Fin de journée)",
 "en": " Reset Cash Register (End of Day)"
 },
 "reset_warning": {
,"!هذا الزر سيحفظ ملخص اليوم ويصفر العداد. استخدمه فقط في نهاية اليوم " :"ar "
 "fr": " Ce bouton enregistrera le résumé du jour et remettra le compteur à zéro. À utiliser
uniquement en fin de journée!",
 "en": " This button will save today's summary and reset the counter. Use only at end of day!"
 },
 "reset_button": {
,"تصفير الخزينة لليوم " :"ar "
 "fr": " Réinitialiser la Caisse du Jour",
 "en": " Reset Today's Cash Register"
 },
 "confirm_reset": {
,".هل أنت متأكد من تصفير الخزينة؟ سيتم حفظ ملخص اليوم في السجل❌" :"ar "
...
,"تقليل مبلغ الدين " :"ar "
 ...
 "reduce_credit_info": {
,"هذا الزر يقلل من مبلغ الدين بدون حذفه كاملً" :"ar "
...
,"سجل المدفوعات " :"ar "
 ...
 "no_credits": {
,"ال توجد ديون حالياً" :"ar "
 "fr": "Aucun crédit pour le moment",
 "en": "No credits currently"
 },
 "delete_credit": {
,"حذف الدين نهائياً " :"ar "
 "fr": " Supprimer le crédit",
 "en": " Delete Credit Permanently"
 },
 "add_to_credit": {
,"إضافة للدين " :"ar "
...
,"الطلبيات " :"ar "
 ...
 "error_generic": {
,".حدث خطأ. الرجاء المحاولة مرة أخرى❌" :"ar "
 "fr": "❌Une erreur est survenue. Veuillez réessayer.",
 "en": "❌An error occurred. Please try again."
 },
 "fill_all_fields": {
,"الرجاء ملء جميع الحقول " :"ar "
 "fr": " Veuillez remplir tous les champs",
 "en": " Please fill all fields"
 },
 "product_added": {
,"!تم إضافة المنتج بنجاح✅" :"ar "
 "fr": "✅Produit ajouté avec succès!",
 "en": "✅Product added successfully!"
 },
 "product_updated": {
,"!تم تحديث المنتج بنجاح✅" :"ar "
...
,"ال توجد بيانات" :"ar "
 ...
 "invoice_printed": {
,"تمت طباعة الفاتورة تلقائياً " :"ar "
...
,"الكمية / العدد" :"ar "
 ...
 "service_confirm": {
,"إتمام الخدمة وطباعة الفاتورة✅" :"ar "
...
"""البحث عن من تج بالباركود أو االسم"""
 if code_or_name:
...
"""تصفير الخزينة وحفظ ملخص اليوم - ثم مسح جميع بيانات اليوم"""
 ...
 total_ventes = df_ventes['Total'].sum() if not df_ventes.empty and 'Total' in df_ventes.columns
else 0
 total_impressions = df_impressions['Total'].sum() if not df_impressions.empty and 'Total' in
df_impressions.columns else 0
 total_jour = total_ventes + total_impressions

باش تجنب الخطأ except/try حفظ ملخص الي وم في التاريخ - استخدم #
 try:
...
def reduce_credit(credit_id, montant_reduction):
 ...
 nouveau_montant = 0
 supabase.table("credits").update({"Montant": nouveau_montant}).eq("id",
int(credit_id)).execute()
...
def fast_barcode_scanner_with_qty(input_label, qty_label):
 ...
 let html5QrcodeScanner = new Html5QrcodeScanner("reader", {{ fps: 10, qrbox: 250, facingMode:
"environment" }});
...
"""سكانير السلة التلقائية - يضيف المنتج للسلة تلقائياً مع التعرف على المنتج الجديد"""
 ...
 let html5QrcodeScanner = new Html5QrcodeScanner("auto_cart_reader", {fps: 10, qrbox: 250,
facingMode: "environment"});
 html5QrcodeScanner.render(onScanSuccess);
 </script>
 """
...
"""ماسح باركود يمأل خانة محددة فقط - يعمل بشكل صحيح"""
 scanner_html = f"""
...
"""مكون التحكم الصوتي"""
 ...
 <button id="start-voice" style="padding:10px 20px; background:#4CAF50; color:white;
border:none; border-radius:5px; cursor:pointer; font-size:16px;">
ابدأ االستماع
 </button>
 <button id="stop-voice" style="padding:10px 20px; background:#f44336; color:white;
border:none; border-radius:5px; cursor:pointer; font-size:16px; display:none;">
إيقاف
 </button>
 ...
 document.getElementById('voice-status').innerText = ' االستماع جاري...;'
 });
 ...
 buttons.forEach(btn => {
 if (btn.innerText.includes('فاتورة || ('btn.innerText.includes('Facture') ||
btn.innerText.includes('طباعة} (('
 btn.click();
 }
 });
...
'األدوات': ' أدوات سريعة'
 ...
 document.getElementById('voice-result').innerText = ' ' + command + ' → ' + value;
 break;
 }
 }
 }
...
إحصائيات سريعة #
 ...
 df_v_today = df_v[df_v['Date'].str.contains(today, na=False)] if not df_v.empty and 'Date' in
df_v.columns else pd.DataFrame()
 total_today = df_v_today['Total'].sum() if not df_v_today.empty and 'Total' in df_v_today.columns
else 0
 ...
 with col1:
 st.metric(" اليوم مبيعات", f"{total_today:.0f} DH")
 with col2:
 st.metric(" طباعة", f"{total_print:.0f} DH")
 with col3:
 st.metric(" المنتجات", len(df_s))
 with col4:
 st.metric(" منخفض مخزون", low_stock)

 st.divider()

========== المنتجات األكثر ربحية ========== #
 st.subheader(t("top_products"))
 if not df_v.empty:
 top_products = df_v.groupby('Nom')['Total'].sum().sort_values(ascending=False).head(10)
 if not top_products.empty:
 fig = px.bar(x=top_products.index, y=top_products.values, title="منتجات 10 أفضل", labels={'x':
({'(DH (المبيعات' :'y ','المنتج '
...
========== مقارنة الفترات ========== #
 ...
 p1 = df_v[(df_v['Date_dt'].dt.date >= date1_start) & (df_v['Date_dt'].dt.date <=
date1_end)]['Total'].sum()
 p2 = df_v[(df_v['Date_dt'].dt.date >= date2_start) & (df_v['Date_dt'].dt.date <=
date2_end)]['Total'].sum()

 col_m1, col_m2, col_m3 = st.columns(3)
 with col_m1:
 st.metric("1 الفترة", f"{p1:.0f} DH")
 with col_m2:
 st.metric("2 الفترة", f"{p2:.0f} DH")
 with col_m3:
 diff = p1 - p2
...
========== رسم بياني للمبيعات ========== #
 ...
 daily_sales.columns = ['المبيعات ','التاريخ['

 fig = px.line(daily_sales.tail(30), x='التاريخ', y='المبيعات', title=")يوم 30 آخر )المبيعات تطور("
 st.plotly_chart(fig, use_container_width=True)
...
========== توقعات المبيعات ========== #
 ...
 with col_t1:
 st.metric(")أيام 7 )المبيعات متوسط", f"{avg_sales:.0f} DH")
 with col_t2:
 st.metric("يوم آخر", f"{last_sales:.0f} DH")
 with col_t3:
 prediction = (avg_sales * 0.7 + last_sales * 0.3)
 st.metric("الغد توقعات", f"{prediction:.0f} DH")

("آخر المبيعات ")subheader.st
 if not df_v.empty:
...
========== نظام التحكم الصوتي ========== #
 ...
 st.success(" Mode Auto: Scannez un produit = Vente directe + Facture 80mm imprimée
automatiquement")
...
حفظ رقم الفاتورة #
 facture_result = generate_facture_80mm([{"Nom": product.get('Nom', code_auto),
"Quantité": 1, "Prix": float(product['Prix']), "Total": total}], "FACTURE DE VENTE")
 ...
 st.success(f"✅{product.get('Nom', code_auto)} - {total:.2f} DH | {t('invoice_number')}:
{invoice_number}")
 st.balloons()
 ...
 st.error(f"❌Stock épuisé pour {product.get('Nom', code_auto)}! Quantité disponible:
{product['Quantité']}")
 else:
 if code_auto:
 ...
 # Scanner automatique
 use_normal_scanner = st.checkbox(" التلقائي الماسح تفعيل", key="normal_scanner_toggle")
 if use_normal_scanner:
(" سيتم كتابته تلقائياً امسح الباركود اآلن - ")info.st
...
رقم الفاتورة #
 facture_result = generate_facture_80mm([{"Nom": nom, "Quantité": qty, "Prix": prix,
"Total": total}], "FACTURE DE VENTE")
 facture_path, invoice_number = facture_result
 ...
 # Scanner automatique
 use_qr_scanner = st.checkbox(" التلقائي الماسح تفعيل", key="qr_scanner_toggle")
 if use_qr_scanner:
(" سيتم كتابته تلقائياً امسح الباركود اآلن - ")info.st
 ...
 facture_result = generate_facture_80mm([{"Nom": nom, "Quantité": qty_qr, "Prix":
prix, "Total": total}], "FACTURE DE VENTE")
 facture_path, invoice_number = facture_result
 ...
 supabase.table("stock").update({"Quantité": q_old - qty_qr}).eq("id",
doc_id).execute()
 play_success_sound()
 st.success(f"{t('sale_success')} {nom} - {qty_qr} x {prix} = {total:.2f} DH | Facture:
{invoice_number}")
 st.rerun()
 else:
 st.error(f"{t('low_stock_warning')} {q_old}")
 ...
 facture_result = generate_facture_80mm([{"Nom": name, "Quantité": qty_libre, "Prix":
float(price), "Total": total_libre}], "FACTURE DE VENTE")
 facture_path, invoice_number = facture_result
...
========== السلة اليدوية ========== #
 ...
 # Scanner automatique
 use_cart_scanner = st.checkbox(" التلقائي الماسح تفعيل", key="cart_scanner_toggle")
 if use_cart_scanner:
(" سيتم كتابته تلقائياً امسح الباركود اآلن - ")info.st
...
التحقق إذا كان المنتج موجوداً مسبقاً في السلة #
 ...
 facture_result = generate_facture_80mm(st.session_state.cart, "FACTURE DE
VENTE")
 facture_path, invoice_number = facture_result
...
التعرف على المنتج الجديد تلقائياً #
 ...
 play_success_sound()
 st.success(f"✅{product.get('Nom', code_auto_cart)} - {float(product['Prix']):.2f} DH
ajouté au panier!")
 st.rerun()
 else:
 ...
 st.dataframe(df_cart_display[['Nom', 'Quantité', 'Prix', 'Total']],
use_container_width=True)
 st.metric(t("total"), f"{total_panier:.2f} DH")

 if st.button(" Enregistrer et Imprimer la Facture", type="primary",
use_container_width=True, key="auto_finish_cart"):
 for item in st.session_state.cart:
 product = get_product_info(item['Code'])
 if product:
 supabase.table("stock").update({
 ...
 **item,
 ...
 if st.button(" Vider le panier", use_container_width=True, key="auto_clear_cart"):
 st.session_state.cart = []
 st.rerun()
 else:
...
خانة البحث #
 ...
 key="stock_search",
"...اكتب اسم المنتج أو الباركود للبحث"=placeholder
 )
 ...
 mask_code = df_stock['Code-barres'].str.contains(search_term, case=False, na=False) if 'Codebarres' in df_stock.columns else pd.Series([False] * len(df_stock))
...
إضافة منتج جديد مع ماسح باركود #
 ...
 if use_add_scanner:
(" سيتم كتابته تلقائياً امسح الباركود اآلن - في خانة الباركود ")info.st
 ...
 use_update_scanner = st.checkbox(t("stock_scanner_update"),
key="update_scanner_checkbox")
...
(" سيتم كتابته تلقائياً امسح الباركود اآلن - في خانة الباركود ")info.st
 ...
 total_ventes = df_ventes_caisse['Total'].sum() if not df_ventes_caisse.empty and 'Total' in
df_ventes_caisse.columns else 0
 total_impressions = df_impressions_caisse['Total'].sum() if not df_impressions_caisse.empty and
'Total' in df_impressions_caisse.columns else 0
 total_credits = df_credits_caisse['Montant'].sum() if not df_credits_caisse.empty and 'Montant' in
df_credits_caisse.columns else 0
...
("تحذير: هذا الزر سيحفظ ملخص اليوم ثم يمسح جميع بيانات اليوم )مبيعات، طباعة، ديون( ")error.st
...
("!هل أنت متأكد؟ سيتم مسح جميع بيانات اليوم نهائياً❌")error.st
 col_confirm, col_cancel = st.columns(2)
 with col_confirm:
 if st.button(t("yes_reset")):
 try:
 total_jour = reset_caisse()
 st.success(f"✅{t('reset_success')} {total_jour:.2f} DH - اليوم بيانات جميع مسح تم("
 ...
 total_historique = df_historique['Total_Jour'].sum() if 'Total_Jour' in df_historique.columns else
0
...
========== بحث في الديون ========== #
("بحث عن دين ")subheader.st
 search_credit = st.text_input(
,":ابحث باسم العميل "
,"...اكتب اسم العميل للبحث"=placeholder
...
تصفية حسب البحث #
 ...
 if df_credits.empty:
 st.info(f"لـ نتائج توجد ال}' search_credit}'")
 else:
 st.success(f"✅على العثور تم} len(df_credits)} نتيجة("
...
========== أزرار: إضافة / تسديد / حذف 3 ========== #
 col_add, col_pay, col_delete = st.columns(3)

زر إضافة للدين )يزيد المبلغ( #
 with col_add:
 if st.button(t("add_to_credit"), use_container_width=True,
key="add_to_existing_credit_btn"):
 ...
 st.success(f"✅إضافة تمت} montant_operation:.2f} DH | الحالي الدين:} nouveau:.2f} DH")
 play_success_sound()
 st.rerun()
 except Exception as e:
 st.error(f"{t('error_generic')}: {str(e)}")
...
بمبلغ **{['Client['data_credit **{هل أنت متأكد من حذف دين "f(warning.st
**{credit_data['Montant']:.2f} DH** ؟("

 col_confirm_del, col_cancel_del = st.columns(2)
 with col_confirm_del:
 if st.button("✅احذف ،نعم", key="confirm_delete_credit_btn"):
 supabase.table("credits").delete().eq("id", credit_id).execute()
("تم حذف الدين نهائياً✅"f(success.st
 play_success_sound()
 time.sleep(0.5)
 st.rerun()
 with col_cancel_del:
...
("اختر ديناً
 ...
 with col_a:
 qte_commande = st.number_input(t("requested_qty"), min_value=0.0, step=0.1,
key="cmd_qte")
 with col_b:
 ...
 commandes_en_attente.apply(lambda x: f"ID: {x['id']} - {x['Nom']} ({x['Qté']} unités)",
axis=1).tolist()
 )
...
"...أدخل اسم الخدمة"=placeholder
 ...
 st.success(f"✅إضافة تمت:} new_service_name} - {new_service_price:.2f} DH")
 play_success_sound()
 time.sleep(0.5)
 st.rerun()
 except Exception as e:
...
(".ال توجد خدمات. أضف خدمات جديدة من األعلى")info.st
...
زر إتمام الخدمة وطباعة الفاتورة #
 if st.button(t("service_confirm"), type="primary", use_container_width=True,
key="service_confirm_btn"):
...
رسالة نجاح #
 st.success(f"✅الخدمة إتمام تم:} st.session_state.selected_service} - {total_service:.2f} DH |
Facture: {invoice_number}")
 st.balloons()
...
("(Online (فتح التطبيقات في المتصفح ")info.st
 ...
 <button style="width:100%; padding:10px; border-radius:10px; border:1px solid #ddd;
background:#185abd; color:white; cursor:pointer; font-size:16px;">
 Word Online
 </button>
 </a>
 ...
 <button style="width:100%; padding:10px; border-radius:10px; border:1px solid #ddd;
background:#107c41; color:white; cursor:pointer; font-size:16px;">
 Excel Online
 </button>
 </a>
 """, unsafe_allow_html=True)
 ...
 <button style="width:100%; padding:10px; border-radius:10px; border:1px solid #ddd;
background:#b7472a; color:white; cursor:pointer; font-size:16px;">
 PowerPoint Online
 </button>
 </a>
 """, unsafe_allow_html=True)
 ...
 <button style="width:100%; padding:10px; border-radius:10px; border:1px solid #ddd;
background:#ea4335; color:white; cursor:pointer; font-size:16px;">
 Gmail
 </button>
 </a>
 """, unsafe_allow_html=True)
...
key="open_whatsapp_btn"):
 whatsapp_url =
f"https://web.whatsapp.com/send?phone={whatsapp_number}&text={whatsapp_message}"
 components.html(f"""
 <script>
 window.open('{whatsapp_url}', '_blank');
 </script>
 """)
 st.success(f"✅فتح تم WhatsApp Web للرقم} whatsapp_number}")
...
key="search_google_btn"):
 ...
 """)
 st.success(f"✅عن البحث تم:} google_query}")

========== مدمج في الصفحة Google ========== #
 st.markdown("---")
