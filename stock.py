...
def generate_pdf(text):
    ...
    pdf.cell(200, 10, txt="Facture Ouzoud", ln=True, align='C')
...
if "invoices_db" not in st.session_state: 
    st.session_state.invoices_db = pd.DataFrame(columns=["Détails", "Total"])
    ...
    if mode == "Vente Normale":
        if not st.session_state.inventory.empty:
            ...
            if st.button("Valider"):
                ...
                st.session_state.invoices_db = pd.concat([st.session_state.invoices_db, pd.DataFrame([[st.session_state.last_receipt, total]], columns=["Détails", "Total"])])
        ...
        if st.button("Valider Vente Libre"):
            ...
            st.session_state.last_receipt = f"--- FACTURE OUZOUD ---\nProduit: {desc}\nTotal: {prix_l} DH"
            st.session_state.invoices_db = pd.concat([st.session_state.invoices_db, pd.DataFrame([[st.session_state.last_receipt, prix_l]], columns=["Détails", "Total"])])
            st.success("Vente Libre enregistrée !")
    
    if st.session_state.last_receipt:
        st.text_area("Dernière Facture:", value=st.session_state.last_receipt, height=150)
        st.download_button("📥 Télécharger la Facture", st.session_state.last_receipt, file_name="facture.txt")
        st.download_button("🖨️ Print PDF", generate_pdf(st.session_state.last_receipt), file_name="facture.pdf", mime="application/pdf")
